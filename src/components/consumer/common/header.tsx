    ...
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 transition-colors duration-500">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 animate-slide-right-md">
            ...
            <span className="text-xl font-bold text-gray-900 dark:text-white">Xegality</span>
          </Link>
            ...
            {CONSUMER_NAVLINKS.map(({ title, href }, index) => {
              ...
              return (
                <Link
                  key={index}
                  href={href}
                  className={cn("text-gray-700 py-2 px-5 rounded-full dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 text-sm font-medium relative group", isActive && "bg-indigo-500/8 text-indigo-500")}
                >
                  {title}
        ...
        {mobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col space-y-4">
              {["Find Lawyers", "Legal Services", "For Lawyers", "About"].map((item, index) => (
                <motion.a
                  key={item}
                  href="#"
